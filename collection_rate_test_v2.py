...
class UDPReceiver:
    ...
    #def _receive_loop(self):
     #   pipeline = (
      #      f"udpsrc port={self.port} "
       #     f"caps=\"application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)JPEG, payload=(int)96\" ! "
        #    f"rtpjpegdepay ! jpegdec ! videoconvert ! "
         #   f"videoscale ! video/x-raw,width=2026,height=1520,format=BGR ! "
...
def main():
    parser = argparse.ArgumentParser(description="영상 수집율 시험 평가 v2")
