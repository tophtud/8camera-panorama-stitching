...
class CuPyStitcher:
    ...
    def stitch_gpu(self, images):
        ...
        for i in range(self.num_cameras):
            img_gpu = cp.asarray(images[i].astype(np.float32))
            xmap = self.gpu_xmaps[i]
            ymap = self.gpu_ymaps[i]
            mask = self.gpu_masks[i]
            ...
            pano[dst_y1:dst_y1 + copy_h, dst_x1:dst_x1 + copy_w] += \
                warped_region * mask_region[:, :, cp.newaxis]
