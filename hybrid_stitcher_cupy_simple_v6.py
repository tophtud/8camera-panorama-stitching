#!/usr/bin/env python3
"""
하이브리드 파노라마 스티칭 v6 (CuPy GPU 가속 - 간소화)
- 폴더 이미지로 캘리브레이션 (OpenCV Stitcher)
- UDP 실시간 스트리밍
- CuPy GPU 가속 (호모그래피 기반)
"""
...
try:
    ...
    print("[Warning] CuPy를 찾을 수 없음. CPU 모드로 동작합니다 (느림).")
...
def crop_edges(images, crop_config):
    ...
    for img in images:
        h, w = img.shape[:2]
        cropped.append(img[crop_top:h-crop_bottom, crop_left:w-crop_right])
...
class SimpleCuPyStitcher:
    """간소화된 CuPy GPU 스티칭 (OpenCV Stitcher 사용)"""
    ...
    def stitch(self, images):
        ...
        # 스티칭
        status, pano = self.stitcher.stitch(images_scaled)
...
class HybridStitcherSimpleV6:
    ...
    def __init__(self, args):
        ...
        # 프레임 스킵
        self.frame_skip = 0
        if args.target_fps >= 10:
            self.skip_interval = 2  # 15 FPS → 7.5 FPS
        else:
    ...
    def calibrate_from_folder(self):
        ...
        # 스티처 초기화
        self.stitcher = SimpleCuPyStitcher(scale=self.args.scale)
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # 정보 표시
                info_text = f"FPS: {self.stitch_fps:.1f} | Frame: {frame_idx} | Simple v6"
                ...
                cv2.imshow('Panorama Simple v6', display)
...
def main():
    parser = argparse.ArgumentParser(description="하이브리드 파노라마 스티칭 v6 (간소화)")
    ...
    parser.add_argument("--save_reference", type=str, default="reference_simple_v6.jpg")
    parser.add_argument("--fps", type=int, default=10)
    
    args = parser.parse_args()
    
    print("=" * 60)
    print("하이브리드 파노라마 스티칭 v6 (간소화)")
    print("=" * 60)
    ...
    stitcher = HybridStitcherSimpleV6(args)
    stitcher.run()
